...
Rules:
1. Build between two and six agents. The first agent is always the Front Desk: it greets, answers general questions, and routes to the others. Every agent is the manager of its own area, so every title ends in "Manager" (for example Front Desk Manager, Billing Manager, Returns Manager). Do not add a separate general manager or supervisor agent; the highest-ranking agent on the team takes escalations, and a person at the business comes after that.
...
export async function generateTeam(profile) {
  ...
  const used = new Set();
  const picked = data.agents.slice(0, 6).map((a, i) => {
