...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ business: { id: biz.id, status: biz.status, phone_number: biz.phone_number }, settings: biz.settings || {}, channels: biz.channels || {}, status: channelStatus(biz) });
  } catch (e) {
